<script lang="ts">
  ...
  const allActivities: Activity[] = [
    {
      ...
      category: "TECHNICAL",
      description: "A 36-hour coding challenge open to all undergraduate students. Build innovative solutions for real-world problems across domains.",
      credits: 15,
      mode: "Offline",
    ...
    },
    {
      ...
      category: "SPORTS",
      description: "Annual inter-college athletics championship. Compete in track and field events representing IIPS at the university level.",
      credits: 10,
      mode: "Offline",
    ...
    },
    {
      ...
      category: "RESEARCH",
      description: "Prestigious national-level science competition covering physics, chemistry, and biology. Written exam followed by practical rounds.",
      credits: 20,
      mode: "Hybrid",
    ...
    },
    {
      ...
      description: "Parliamentary-style debate on contemporary socio-political topics. Individual and team participation categories available.",
      credits: 12,
      mode: "Offline",
    ...
    },
    {
      ...
      category: "CULTURAL",
      description: "Classical and contemporary dance competition as part of the Annual Cultural Festival. Solo and group categories.",
      credits: 10,
      mode: "Offline",
  ...
  const deadlines = [
    ...
    { name: "Debate Championship", days: "5d left", color: "bg-amber-50 text-amber-700 border-amber-100" },
    { name: "Athletics Meet", days: "7d left", color: "bg-emerald-50 text-emerald-700 border-emerald-100" },
    { name: "Blood Donation Camp", days: "9d left", color: "bg-blue-50 text-blue-700 border-blue-100" }
  ];
  ...
  let creditsEarned = $derived(
    baseCreditsEarned + enrolledIds.reduce((total, id) => {
  ...
  let filteredActivities = $derived.by(() => {
    return allActivities.filter(activity => {
      ...
      if (activeFilters.credits) {
        if (activeFilters.credits === "1-10" && (activity.credits < 1 || activity.credits > 10)) return false;
        if (activeFilters.credits === "11-15" && (activity.credits < 11 || activity.credits > 15)) return false;
        if (activeFilters.credits === "16-20" && (activity.credits < 16 || activity.credits > 20)) return false;
      }
  ...
  <div class="lg:col-span-9 space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs">
      <div class="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider font-sans">Find Activities</h3>
        <button 
        ...
        <div transition:slide={{ duration: 200 }} class="space-y-4">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            ...
            <div class="flex flex-col gap-1.5">
              <label for="filter-category" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Category</label>
              <select 
                id="filter-category"
            ...
            <!-- Status -->
            <div class="flex flex-col gap-1.5">
              <label for="filter-status" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Status</label>
              <select 
                id="filter-status"
            ...
            <!-- Credits Range -->
            <div class="flex flex-col gap-1.5">
              <label for="filter-credits" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Credits Range</label>
              <select 
                id="filter-credits"
            ...
            <!-- Mode -->
            <div class="flex flex-col gap-1.5">
              <label for="filter-mode" class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mode</label>
              <select 
                id="filter-mode"
    ...
    <div class="space-y-2.5">
      <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider font-sans">
        Recommended For You <span class="text-[10px] text-slate-400 normal-case font-medium">Based on your enrolled tracks</span>
      </h3>
      <div class="flex flex-wrap gap-2.5">
        ...
        >
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 21l-.813-5.096L3 15l5.096-.813L9 9l.813 5.096L15 15l-5.187.904zM18 5.25L17.25 8 16.5 5.25 13.75 4.5l2.75-.75L17.25 1l.75 2.75 2.75.75-2.75.75z" />
          </svg>
          Technical Skill Track <span class="bg-[#881B1B] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">6</span>
        </button>
        <button 
        ...
        >
          <!-- Beaker/Science Icon -->
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.104v11.896m5.25-11.896v11.896M2.25 21h19.5M10 3.75h4" />
          </svg>
          Research & Innovation <span class="bg-purple-700 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">4</span>
        </button>
        <button 
        ...
        >
          <!-- Mic Icon -->
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 0 3-3V4.5a3 3 0 0 0-6 0v8.25a3 3 0 0 0 3 3z" />
          </svg>
          Public Speaking <span class="bg-amber-700 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">3</span>
        </button>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <div class="flex items-baseline justify-between border-b border-slate-200 pb-2">
        <h2 class="text-base font-bold font-serif text-[#0B1535]">All Activities <span class="text-xs font-semibold text-slate-400 normal-case ml-2">{filteredActivities.length} found</span></h2>
      </div>

      {#if paginatedActivities.length > 0}
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {#each paginatedActivities as activity (activity.id)}
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col justify-between shadow-xs hover:shadow-md transition-all duration-200 relative group">
              ...
              <div class="p-5 flex-grow space-y-4">
                ...
                <div class="flex justify-between items-center">
                  <span class="inline-flex px-2 py-0.5 rounded border text-[9px] font-bold uppercase tracking-wider {getCategoryBadgeColor(activity.category)}">
                    {activity.category}
                  </span>
                  
                  {#if activity.status === 'Closing Soon'}
                    <span class="text-[9px] font-bold text-rose-600 bg-rose-50 border border-rose-100 px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Closing Soon
                    </span>
                  {/if}
                </div>
                ...
                <!-- Title & Description -->
                <div>
                  <h4 class="text-xs font-bold text-[#0B1535] leading-snug group-hover:text-[#881B1B] transition-colors">{activity.name}</h4>
                ...
                <!-- Details List -->
                <div class="space-y-2 pt-3 border-t border-slate-100 text-[11px] font-medium text-slate-500">
                  <!-- Credits -->
                  <div class="flex items-center gap-2">
                    ...
                    <span>Credits: <strong class="text-slate-800">{activity.credits} Points</strong></span>
                  </div>

                  <!-- Mode -->
                  <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 text-slate-400">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 17L9 20l-1 3h4l-.5-3 .5-3m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25" />
                    </svg>
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span>Registration Deadline: <strong class="text-slate-800">{activity.regDeadline}</strong></span>
                  </div>
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span>Activity Date: <strong class="text-slate-800">{activity.activityDate}</strong></span>
                  </div>
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span>Coordinator: <strong class="text-slate-800">{activity.coordinator}</strong></span>
                  </div>
                </div>
              </div>
        ...
        <!-- Pagination Controls -->
        {#if totalPages > 1}
          <div class="flex items-center justify-center gap-2 pt-6 border-t border-slate-200 mt-6 font-sans">
            <button 
            ...
            {#each Array(totalPages) as _, index}
              <button 
                ...
                class="w-8 h-8 rounded-lg text-xs font-bold transition flex items-center justify-center border {currentPage === index + 1 ? 'bg-[#0B1535] border-[#0B1535] text-white' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}"
              >
                {index + 1}
              </button>
            {/each}
      ...
      {:else}
        <div class="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-xs">
          <p class="text-slate-400 font-semibold text-xs">No activities found matching the filters.</p>
          <button 
            onclick={handleClearFilters}
  ...
  <div class="lg:col-span-3 space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
      <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider font-sans">Registration Deadlines</h3>
      </div>
      <div class="space-y-3 pt-1">
        {#each deadlines as item}
          <div class="flex items-center justify-between text-xs font-semibold text-slate-800">
            ...
            <span class="px-2 py-0.5 rounded text-[10px] font-bold border shrink-0 font-sans {item.color}">
              {item.days}
            </span>
          </div>
        {/each}
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
      <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider font-sans">My Enrollment Status</h3>
      </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
      <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider font-sans">Activity Categories</h3>
      </div>
      <div class="space-y-2.5 pt-1">
        {#each categoriesCounts as cat}
          ...
          >
            ...
            <span class="bg-slate-100 text-slate-700 font-bold text-[10px] px-2 py-0.5 rounded-full font-sans">
              {cat.count}
            </span>
          </button>
        {/each}
